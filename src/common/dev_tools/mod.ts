...
import { relative, common } from "path"
...
function createMessageEnvelope<Payload>(type: MessageType, payload: Payload): MessageEnvelope<Payload> {
...
export class HotSwapServer {
  constructor(private client: WebSocket) {
    ...
    addModifyFileListener(this.handleModifyFile.bind(this), [Deno.cwd() + "/public"] )
  }
...
export async function addModifyFileListener(listener: FsListener, absPaths: Array<string>) {
  ...
  listenerCount++
  console.log(`Starting modify file listener #${listenerCount} for ${absPaths.join(', ')}`)
