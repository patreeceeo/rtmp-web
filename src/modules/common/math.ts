...
export function clampLine(start: Vec2, end: Vec2, maxLength: number): Vec2 | null {
  const {x: x1, y: y1} = start
  const {x: x2, y: y2} = end
  const dx = x2 - x1;
  const dy = y2 - y1;
  const distanceCircleToLine = Math.abs(dy * x1 - dx * y1 - x1 * y2 + x2 * y1) / Math.sqrt(dx * dx + dy * dy);
  ...
  const a = Math.sqrt(maxLength * maxLength - distanceCircleToLine * distanceCircleToLine);
  const t = a / length;
